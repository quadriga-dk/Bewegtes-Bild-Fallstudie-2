...
Durch die *Cluster*-Funktion hat sich in der Spalte *Jahr* eine Mischung aus den Datentypen *number* (in grüner Farbe angezeigt) und *text* (in schwarzer Farbe angezeigt) ergeben. Bei den weiteren Bereinigungsschritten kann dies zu Problemen und Inkonsistenzen führen. Wandeln Sie daher die komplette Spalte zunächst in *text* um, die Umwandlung in *number* wurde bereits im Abschnitt zu [Numeric Facets](/bereinigung/openRefine/3_sichtung.md#numeric-facet) beschrieben. Analog Wählen Sie in der Spalte *Jahr* den Menüpunkt *Edit cells* aus, dann *Common transforms*, gefolgt von *To text*. Alle Einträge der Spalte liegen nun als Datentyp *text* vor.
