...
Grundsätzlich ist es möglich, mit der [im vorigen Abschnitt](5.1_jahre.md) vorgestellten Methode die Titel-Einträge nach jedem einzelnen Artikel zu filtern und diesen an das Ende der entsprechenden Einträge zu setzen. Jedoch erzeugt ein solcher manueller Ansatz viele *OpenRefine* Edits und ist sehr fehleranfällig – insbesondere bei großen Datensätzen in mehreren Sprachen. Stattdessen empfiehlt es sich, ein Muster bezüglich des Auftretens der Artikel zu bestimmen und sie alle anhand dieses Musters in einem Schritt zu verarbeiten.
...
:class: keypoint
Der folgende Text enthält einige Code-Beispiele zur Veranschaulichung des Vorgehens und der GREL- bzw. Regex-Elemente. Wenn Sie möchten, können Sie jedes der Beispiele auch in ihrem eigenen Projekt ausprobieren. Selbst anwenden müssen Sie allerdings nur den vollständigen Code, der in einem Code Block außerhalb des Fließtextes angezeigt wird. Ein Code Block sieht folgendermaßen aus:
...
Für die [Eingabe von *GREL*-Code](/bereinigung/openRefine/5.1_jahre.md#grel) mit *Regex*-Mustern öffnen Sie wiederum das Dropdown-Menü einer Spalte, den Reiter *Edit cells* und *Transform…* und gelangen so in das entsprechende Menü.
...
Für die folgenden Screenshots wurde die Spalte *Titel* mittels eines *Text Filter* nach den Nichtsortierzeichen `<<  >>` gefiltert (siehe [Text Filter](./3_sichtung.md#text-filter)).
...
Fällt Ihnen auf, wie die Erfassungsklammern verschoben wurden, um auch die Nichtsortierzeichen zu entfernen?
...
Den Erfassungsklammern wurde die Zeichenfolge `(?i)` vorangestellt. Dadurch wird das match *case-insensitive*: Groß- und Kleinschreibung werden bei der Suche nach den Artikeln nicht beachtet. Das Leerzeichen `\s` wurde hinter die Erfassungsklammer verschoben – somit muss nicht jedem Artikel noch ein `\s` hinzugefügt werden. Zudem wurde die Methode <a href="https://openrefine.org/docs/manual/grelfunctions#trims" class="external-link" target="_blank">`trim`</a>
 am Ende der ersten Zeile des Codes ergänzt. Diese entfernt Leerzeichen am Anfang und Ende des Titels.
...
Mit der Anwendung des *GREL*-Codes sind die Artikel bei den Filmtiteln nun nachgestellt und großgeschrieben. Die Spalte *Titel* ist somit in dieser Hinsicht erfolgreich bereinigt.