...
```{admonition} Hinweis
:class: hinweis
Sie müssen nicht jeden der folgenden Schritte für ihr eigenes Projekt anwenden - die letzten zwei Code-Snippets genügen.
```
````
...
```{admonition} Hinweis
:class: hinweis
Für die folgenden Screenshots wurde die Spalte mittels eines *Text Filter* nach den Vergleichsklammern `<<  >>` gefiltert. 
```
````
...
alt: Anwendung des Regex-Codes value.replace(value.match(/(<<.*>>).*/)[0], "")
---
Anwendung des *Regex*-Codes `value.replace(value.match(/(<<.*>>).*/)[0], "")`
```
